...
long dispatch_ioctl(struct file *const file, unsigned int const cmd,
  ...
  case OP_READ_MEM: {
    if (copy_from_user(&cm, (void __user *)arg, sizeof(cm)) != 0) {
      return -1;
      printk("hack_zbw: copy_from_user fail");
    }
    printk("hack_zbw: copy_from_user success");
    if (read_process_memory(cm.pid, cm.addr, cm.buffer, cm.size) == false) {
      return -1;
      printk("hack_zbw: read_process_memory fail");
    }
  } break;
  case OP_WRITE_MEM: {
    if (copy_from_user(&cm, (void __user *)arg, sizeof(cm)) != 0) {
...
int __init driver_entry(void) {
  int ret;
  printk("hack_zbw: [+] driver_entry");
  ret = misc_register(&misc);
  return ret;
}

void __exit driver_unload(void) {
  printk("hack_zbw: [+] driver_unload");
